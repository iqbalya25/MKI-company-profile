...
export const productStyles = {
  ...
  card: {
    // Container styles
    container: "bg-white border border-gray-200 rounded-lg overflow-hidden hover:shadow-xl transition-all duration-300 group",
    containerList: "bg-white border border-gray-200 rounded-lg p-6 hover:shadow-lg transition-all duration-300 group",
    ...
    header: {
      ...
      title: "font-semibold text-lg text-gray-900 mb-2 line-clamp-2 group-hover:text-teal-600 transition-colors",
      titleList: "font-semibold text-xl text-gray-900 mb-3 group-hover:text-teal-600 transition-colors line-clamp-2",
    ...
    // Status badges
    badges: {
      container: "absolute top-3 left-3 right-3 flex items-start justify-between",
    ...
    // Overlay effects
    overlay: {
      container: "absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-all duration-300 flex items-center justify-center opacity-0 group-hover:opacity-100",
  ...
  search: {
    // Main container
    container: "bg-white border border-gray-200 rounded-lg shadow-sm p-6 mb-8",
    ...
    header: {
      ...
      stats: "text-right",
      statsNumber: "text-2xl font-bold text-teal-600",
    ...
    filters: {
      ...
      badge: "inline-flex items-center gap-1 px-2 py-1 bg-teal-100 text-teal-800 text-xs rounded-full",
      removeButton: "hover:bg-teal-200 rounded-full p-0.5",
      removeIcon: "h-3 w-3",
      clearButton: "text-teal-600 border-teal-300 hover:bg-teal-50",
    },
    ...
    form: {
      container: "space-y-4",
      inputGroup: "relative",
      input: "w-full h-10 pl-10 pr-4 border border-gray-300 rounded-lg bg-white text-gray-700 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent transition-colors",
      inputIcon: "absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-gray-400",
      selectGroup: "grid grid-cols-1 md:grid-cols-2 gap-4",
      select: "w-full h-10 pl-4 pr-10 border border-gray-300 rounded-lg bg-white text-gray-700 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent appearance-none cursor-pointer transition-colors",
      selectIcon: "absolute right-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-gray-400 pointer-events-none",
      button: "w-full h-10 bg-gradient-to-r from-teal-600 to-teal-500 hover:from-teal-700 hover:to-teal-600 text-white font-medium shadow-md hover:shadow-lg transition-all",
  ...
  // 🛠️ TOOLBAR STYLES
  toolbar: {
    container: "flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6 p-4 bg-white border border-gray-200 rounded-lg",
    ...
    sortSelect: "text-sm border border-gray-300 rounded-md px-3 py-1 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent",
    viewToggle: "flex items-center border border-gray-300 rounded-md overflow-hidden",
  ...
  // 📄 PAGINATION STYLES
  pagination: {
    container: "mt-12 flex flex-col sm:flex-row items-center justify-between gap-4",
    info: "text-sm text-gray-600",
    controls: "flex items-center gap-2",
    button: "inline-flex items-center px-3 py-2 text-sm font-medium rounded-md border",
...
export const getGridClasses = (viewMode: 'grid' | 'list' = 'grid') => {
  return viewMode === 'grid' ? productStyles.grid.container : productStyles.grid.list;
};

export const getCardClasses = (viewMode: 'grid' | 'list' = 'grid', featured = false) => {
  const base = viewMode === 'grid' ? productStyles.card.container : productStyles.card.containerList;
  return featured ? `${base} ring-2 ring-teal-200 ring-offset-2` : base;
};

export const getStatusClasses = (status: 'inStock' | 'outStock' | 'featured') => {
  switch (status) {
    case 'inStock': return `${productStyles.status.badge} ${productStyles.status.inStock}`;
    case 'outStock': return `${productStyles.status.badge} ${productStyles.status.outStock}`;
    case 'featured': return `${productStyles.status.badge} ${productStyles.status.featured}`;
    default: return productStyles.status.badge;
  }
};